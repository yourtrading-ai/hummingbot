...
class MangoPerpetualAPIDataSource(CLOBPerpAPIDataSourceBase):
    ...
    async def place_order(
        ...
        exchange_order_id = identified_orders[0].get("exchange_order_id")
    ...
    async def get_all_order_fills(self, in_flight_order: GatewayInFlightOrder) -> List[TradeUpdate]:
        ...
        trade_updates = []
        fill_price = Decimal(order["uiPrice"])
        fill_size = in_flight_order.amount - Decimal(order["uiSize"])
        fee_token = "BTC"
    ...
    async def _get_order_status_update_with_order_id(self, in_flight_order: InFlightOrder) -> Optional[OrderUpdate]:
        ...
        if resp.get("orders") == "":
            ...
            status_update = OrderUpdate(
                ...
                new_state=MANGO_DERIVATIVE_ORDER_STATES[orders[0]["state"]],
                client_order_id=in_flight_order.client_order_id,
    ...
    async def _on_cancel_order_transaction_failure(self, order: GatewayInFlightOrder,
                                                   cancelation_result: Dict[str, Any]):
        raise ValueError(
