...
from hummingbot.core.data_type.common import OrderType, SerumOrderType, SerumTradeType, TradeType
...
class SerumInFlightOrder(InFlightOrder):
    def __init__(
        self,
        client_order_id: str,
        trading_pair: str,
    ...
    @classmethod
    def from_json(cls, data: Dict[str, Any]) -> "SerumInFlightOrder":
        """
        Initialize an InFlightOrder using a JSON object
        :param data: JSON data
        :return: Formatted InFlightOrder
        """
        order = SerumInFlightOrder(
            client_order_id=data["client_order_id"],
            trading_pair=data["trading_pair"],
            order_type=getattr(SerumOrderType, data["order_type"]),
            trade_type=getattr(SerumTradeType, data["trade_type"]),
...
class SerumOrderState(Enum):
    OPEN = 1,
    CANCELED = 2,
    FILLED = 3,
    CREATION_PENDING = 4,
    CANCELATION_PENDING = 5,
    UNKNOWN = 6,
