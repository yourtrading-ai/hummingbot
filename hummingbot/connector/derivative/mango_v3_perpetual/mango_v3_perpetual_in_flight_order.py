...
from hummingbot.connector.derivative.mango_v3_perpetual.mango_v3_perpetual_fill_report import MangoV3PerpetualFillReport
from hummingbot.connector.derivative.mango_v3_perpetual.mango_v3_perpetual_order_status import \
    MangoV3PerpetualOrderStatus
from hummingbot.core.event.events import (TradeType, OrderType, MarketEvent)


class MangoV3PerpetualInFlightOrder(InFlightOrderBase):
    def __init__(self,
                 client_order_id: str,
                 ...
                 price: Decimal,
                 amount: Decimal,
                 initial_state: MangoV3PerpetualOrderStatus,
                 filled_size: Decimal,
    ...
    def is_done(self) -> bool:
        return self.status >= MangoV3PerpetualOrderStatus.done

    @property
    def is_cancelled(self) -> bool:
        return self.status in [MangoV3PerpetualOrderStatus.CANCELED, MangoV3PerpetualOrderStatus.expired]

    @property
    def is_failure(self) -> bool:
        return self.status >= MangoV3PerpetualOrderStatus.failed
    ...
    def from_json(cls, data: Dict[str, Any]):
        order = MangoV3PerpetualInFlightOrder(
            ...
            Decimal(data["price"]),
            Decimal(data["amount"]),
            MangoV3PerpetualOrderStatus[data["status"]],
        ...
        for fill in data["fills"]:
            order.fills.add(MangoV3PerpetualFillReport(fill['exchange_id'],
                                                       Decimal(fill['amount']),
                                                       Decimal(fill['price']),
                                                       Decimal(fill['fee'])))
    ...
    def from_mango_v3_order(cls,
                            ...
                            leverage: int,
                            position: str):
        return MangoV3PerpetualInFlightOrder(
            client_order_id,
            hash,
            trading_pair,
            order_type,
            ...
            price,
            amount,
            MangoV3PerpetualOrderStatus.PENDING,
            Decimal(0),
            Decimal(0),
            Decimal(0),
            created_at,
    ...
    def _enqueue_completion_event(self):
        if (not self._completion_sent and
                self.status is MangoV3PerpetualOrderStatus.FILLED and
            ...
            self._queued_events.append(
                (MarketEvent.BuyOrderCompleted if self.trade_type is TradeType.BUY else MarketEvent.SellOrderCompleted,
    ...
    def register_fill(self, id: str, amount: Decimal, price: Decimal, fee: Decimal):
        if id not in self.fills:
            report = MangoV3PerpetualFillReport(id, amount, price, fee)
    ...
    def update(self, data: Dict[str, Any]) -> List[Any]:
        new_status: MangoV3PerpetualOrderStatus = MangoV3PerpetualOrderStatus[data["status"]]

        if not self.is_done:
            if new_status == MangoV3PerpetualOrderStatus.CANCELED:
                self._queued_events.append((MarketEvent.OrderCancelled, None, None, None))
            elif new_status == MangoV3PerpetualOrderStatus.failed:
                self._queued_events.append((MarketEvent.OrderFailure, None, None, None))
                new_status = MangoV3PerpetualOrderStatus.failed
            elif new_status == MangoV3PerpetualOrderStatus.expired:
                self._queued_events.append((MarketEvent.OrderExpired, None, None, None))
                new_status = MangoV3PerpetualOrderStatus.expired
